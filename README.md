...
async function getAttestationSignaturePublicKey(ctx, iss, header, client) {
  ...
  // @param client - client making the request
  throw new Error(
    "features.attestClientAuth.getAttestationSignaturePublicKey not implemented"
  );
}
```
...
const attesters = new Map(
  Object.entries({
    "https://attester.example.com": jose.createRemoteJWKSet(
      new URL("https://attester.example.com/jwks")
    ),
  })
);
  ...
  ctx.body = `<!DOCTYPE html>
    ...
    <body>
      <div>
        ...
        <p>Your sign-in ${
          ctx.oidc.client.clientName ? `with ${ctx.oidc.client.clientName}` : ""
  ...
  ctx.body = `<!DOCTYPE html>
    ...
    <body>
      <div>
        ...
        <p>
          <strong>${
            ctx.oidc.client.clientName || ctx.oidc.client.clientId
          }</strong>
          <br/><br/>
...
```js
new provider.InitialAccessToken({ policies: ["my-policy", "my-policy-2"] })
  .save()
  .then(console.log);
```
...
const TaxData = z
  .object({
    ...
    locations: z
      .array(z.literal("https://taxservice.govehub.no.example.com"))
      .length(1),
...
[
  ...
  "private_key_jwt",
  "tls_client_auth",
  "self_signed_tls_client_auth", // these methods are only available when features.mTLS is configured
];
```
...
const extraParams = {
  async origin(ctx, value, client) {
    ...
    if (!allowedOrigin(value, client)) {
      ...
      throw new errors.InvalidRequest(
        'requested "origin" is not allowed for this client'
      );
    }
  },
};
...
[
  ...
  "ML-DSA-44",
  "ML-DSA-65",
  "ML-DSA-87", // available in Node.js >= 24.7.0
];
```
...
```js
["HS256", "RS256", "PS256", "ES256", "Ed25519", "EdDSA"];
```
